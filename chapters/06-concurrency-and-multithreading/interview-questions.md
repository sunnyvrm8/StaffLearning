...
**Answer:** A **single `sync.Mutex` / `synchronized` map** becomes the hotspot; **GC** pressure from boxed entries and churn hurts Go/Java alike. Mitigations: **shard** by `hash(userId) % N` with per-shard locks; **striped** locks; or **atomic token buckets per shard** with occasional reconciliation. At **multi-instance** scale, in-process limits are **wrong per user** (each node grants full quota)—move to **Redis/sidecar** with sliding window ([Case Study 5: Rate Limiter](../../case-studies/05-rate-limiter/README.md)). First failure mode to name in interviews: **correct on one box, 5× allowance behind a load balancer**.
